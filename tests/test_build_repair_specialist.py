"""tests/test_build_repair_specialist.py -- Integration tests for Task 12.
...
Covers the 11 integration tests specified in ``.superpowers/sdd/task-12-brief.md``:
  ...
  11. test_validation_matches_task11 -- validation.jsonl sample_ids match
...
def generated_train() -> list:
    """Ensure train.jsonl exists and return a list of Sample objects."""
...
def test_train_count(generated_train):
    """train.jsonl has up to 493 samples.

    Issue #10 Fix 1 (PILOT ONLY): train count may be less than 493 if the
...
def test_ratio_within_tolerance(generated_train):
    """All 4 variant ratios within ±3pp of 15/15/30/40.
...
def test_deterministic_sampling(generated_train):
    ...
    For static_repair bucket where count == target (148), take ALL sorted.
...
def test_rejected_count():
    """rejected.jsonl has at least 289 records.

    Issue #10 Fix 1: rejected count may be higher than 289 because
...
def test_manifest_consistency(generated_train, generated_validation, manifest):
    ...
    # pool_source counts (Issue #10 Fix 1: samples_selected may be < 493)
    ...
    # Issue #10 Fix 1: deviations.verified_backfill_applied is True and
    # deviations.history records the legacy normalization context.
    # (Replaces the old deviations.verified_normalization field.)
    assert manifest["deviations"]["verified_backfill_applied"] is True, (
        f"manifest deviations.verified_backfill_applied is "
        f"{manifest['deviations']['verified_backfill_applied']}, expected True"
    )
    assert "history" in manifest["deviations"], (
        "manifest deviations missing 'history' field"
    )
    history = manifest["deviations"]["history"]
    assert history["verification_subfields_preserved"] is True, (
        f"manifest deviations.history.verification_subfields_preserved is "
        f"{history['verification_subfields_preserved']}, expected True"
    )
    # All train samples must have verified=True (Issue #10 Fix 1:
    # enforced by filtering, not by model_copy hack)
    unverified_in_train = [s.sample_id for s in generated_train if not s.verified]
    assert not unverified_in_train, (
        f"{len(unverified_in_train)} train samples have verified=False: "
        f"first 5={unverified_in_train[:5]}"
    )


# ---------------------------------------------------------------------------
# Test 11: validation matches Task 11 (Global Constraint #18)
# ---------------------------------------------------------------------------
...
def test_validation_matches_task11(generated_validation):
    ...
    if not TASK11_VALIDATION_PATH.exists():
        pytest.skip(
            "Task 11 validation.jsonl missing "
            f"({TASK11_VALIDATION_PATH})"
        )
    task11_samples: list = []
    with TASK11_VALIDATION_PATH.open(encoding="utf-8") as fh:
        for line in fh:
            line = line.strip()
            if not line:
                continue
            task11_samples.append(Sample.from_json_line(line))

    task11_ids_sorted = sorted(s.sample_id for s in task11_samples)
    repair_ids_sorted = sorted(s.sample_id for s in generated_validation)

    assert task11_ids_sorted == repair_ids_sorted, (
        f"validation sample_ids differ from Task 11: "
        f"missing_in_repair={sorted(set(task11_ids_sorted) - set(repair_ids_sorted))[:5]} "
        f"extra_in_repair={sorted(set(repair_ids_sorted) - set(task11_ids_sorted))[:5]}"
    )

    # Also verify the order in the file is identical (both sorted by sample_id)
    task11_ids_in_order = [s.sample_id for s in task11_samples]
    repair_ids_in_order = [s.sample_id for s in generated_validation]
    assert task11_ids_in_order == repair_ids_in_order, (
        "validation sample_id order differs from Task 11 (both should be "
