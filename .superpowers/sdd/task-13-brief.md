...
- Produces: split SFT dataset at `data/p4-agent/sft-v1/{train,validation,heldout-agent-eval,failure-diagnostics}.jsonl`, `data/p4-agent/sft-v1/manifest.json`, `reports/p4/sft-dataset-replay-failures.jsonl` — all generated at runtime, NOT committed

**Design note:** Replay uses `_ListActionProvider(actions)`, NOT `ReplayActionProvider(Trajectory)`. For P4.0 scripted trajectories (which ARE in `Trajectory` format), the builder extracts the action list from `traj.steps[i].action` for each step, then uses `_ListActionProvider` for replay. For P4.1 JSONL trajectories (T9/T10/T11/T12), actions are reconstructed from the `actions` list field using `TypeAdapter(Action).validate_python(action_dict)` (SentinelActions are reconstructed via the `__sentinel__` marker).

**Global Constraints:**
1. ONLY create `scripts/build_agent_sft_dataset.py`. Do NOT touch any source files in `src/`.
2. `src/agent_trajectory.py` is FROZEN — do not modify.
3. The script does NOT require GPU (uses _ListActionProvider for replay, no model inference). Only verify syntax via `ast.parse` in this environment. The actual run happens before PR merge (after T9/T10/T11/T12 outputs are generated).
4. `_ListActionProvider` goes INSIDE the script file (defined inline, same pattern as T10/T11/T12).
...
- `Trajectory` has `.trajectory_id: str` (line 86), `.task_id: str` (line 87), `.steps: list[TrajectoryStep]`, `.final_success: bool` (line 91, derived at line 154)
- `TrajectoryStep` has `.action: Action` (line 54)
- `scripted.jsonl` exists at data/p4-agent/trajectories-v0/scripted.jsonl
- `Action` in src/agent_actions.py (line 260, Annotated union alias — NOT a BaseModel, requires TypeAdapter)
- `SentinelAction` in src/agent_model_provider.py (BaseModel with `is_invalid: bool = True`, `reason: str = ""`)
- `AgentEvaluator`, `ActionProvider`, `AgentState` in src/agent_evaluator.py
- `MicroTaskWorkspace.from_task` exists
- `TypeAdapter` from pydantic (already used in src/agent_model_provider.py since T7)

**Pre-flight Correction 1 (Action.model_validate → TypeAdapter):**
The original plan code used `Action.model_validate(d)` in `_reconstruct_actions`. This is the SAME bug as T6/T7: `Action` is `Annotated[Union[...], Field(discriminator="action_type")]` — a typing alias, NOT a Pydantic model class. `Action.model_validate(d)` raises `AttributeError`. Fix:
1. Add `from pydantic import TypeAdapter` to imports (after `from pathlib import Path`)
2. Add module-level `_ACTION_ADAPTER = TypeAdapter(Action)` after the imports (after `_ListActionProvider` class definition, before path constants)
3. Change `Action.model_validate(d)` → `_ACTION_ADAPTER.validate_python(d)` in `_reconstruct_actions`

- [ ] **Step 1: Write the dataset builder script**

Create `scripts/build_agent_sft_dataset.py` with EXACTLY this content (Correction 1 applied):
