...
- Produces: `reports/p4/p4-1-readiness.md` — readiness report with 10 gates + verdict (generated at runtime, NOT committed)

**Global Constraints:**
1. ONLY create `scripts/verify_p4_1_readiness.py`. Do NOT touch any source files in `src/`.
2. `src/agent_trajectory.py` is FROZEN — do not modify.
3. The script does NOT require GPU. It runs pytest tests and checks JSON files. Only verify syntax via `ast.parse` in this environment. The actual run happens before PR merge (after T9/T10/T11/T12/T13 outputs are generated).
...
- `reports/p4/p4-0-baseline-lock.json` — created in T1 (commit 824a5c2), exists
- Test names verified to exist:
  - Gate 02: `test_test_pass_success_uses_replay_not_claim` (line 309), `test_test_pass_mismatch_claimed_fail_actual_pass` (line 332), `test_test_pass_no_mismatch_when_claim_matches_replay` (line 360) — all in test_agent_evaluator.py ✓
  - Gate 03: `test_unknown_action_type_recorded_as_forbidden` (line 394 — NOT `test_unknown_action_type_raises`), `test_allowed_action_types_has_exactly_11` (line 384) — in test_agent_evaluator.py ✓
  - Gate 04: `test_search_text_dispatched` (line 421) — in test_agent_evaluator.py ✓
  - Gate 05: `test_inspect_error_returns_stdout_on_test_failure` (line 525), `test_inspect_error_caps_at_8kb` (line 544) — in test_agent_tools.py ✓
  - Gate 06: `test_corruption_wrong_action_type` (line 477), `test_corruption_invalid_path` (line 486), `test_corruption_wrong_patch` (line 510), `test_corruption_skip_tests_before_finish` (line 531), `test_corruption_exceed_max_steps` (line 556) — all in test_agent_evaluator.py ✓
- `reports/p4/model-trajectory-collection-report.json` — generated by T9 at runtime (won't exist in this env, gate handles gracefully)
- `data/p4-agent/sft-v1/manifest.json` — generated by T13 at runtime (won't exist in this env, gate handles gracefully)

**Pre-flight Correction 1 (test name fix):**
The original plan code referenced `test_unknown_action_type_raises` in `gate_03_unknown_action_hard_fails`. This test does NOT exist. The actual test name is `test_unknown_action_type_recorded_as_forbidden` (at test_agent_evaluator.py line 394). Fix: change the test name in gate 03 from `test_unknown_action_type_raises` to `test_unknown_action_type_recorded_as_forbidden`.

- [ ] **Step 1: Write the readiness verifier**

Create `scripts/verify_p4_1_readiness.py` with EXACTLY this content (Correction 1 applied — test name fixed in gate 03):
